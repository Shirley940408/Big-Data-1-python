...
# (2) rgb2lab_query()：生成 SQL 用于 RGB → LAB 转换， LAB是一个更广阔且更符合人眼感知的颜色空间，它将亮度 (L)、红绿偏向 (a) 和黄蓝偏向 (b) 分离，适合精确的颜色调整和跨设备色彩匹配。
...
# (3) rgb2hsv_query()：RGB → HSV 转换（可选）这个函数和 rgb2lab_query 类似，只是转换成 HSV 空间。
...
def plot_predictions(model, description, lum=70, resolution=256, output_filename=None,
    ...
    """
    Create a slice of LAB colour space with given luminan ce; predict with the model; plot the results.
    """
    wid = resolution
    hei = resolution
    n_ticks = 5
